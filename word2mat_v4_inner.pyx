...
cdef void fast_sentence_sg_hs(
    ...
    REAL_t *syn0L,REAL_t *syn0R, REAL_t *syn1,const int rank, const int vector_size,const int topic_size,
    const REAL_t * context_vector,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work,REAL_t * neu1,REAL_t *tmp, REAL_t *word_locks) nogil:
    cdef int r,v
    cdef long long a, b
    cdef int tp
    cdef long long row1_L = word2_index * vector_size * rank, row2
    cdef long long row1_R = word2_index * rank * topic_size
    cdef REAL_t f, g
    cdef char trans = <char >'c'
    memset(work, 0, vector_size * cython.sizeof(REAL_t))
    memset(neu1, 0,  vector_size * cython.sizeof(REAL_t))
    memset(tmp,0,rank*cython.sizeof(REAL_t))
    for tp in range(topic_size):
        if context_vector[tp] > 0:
            for r in range(rank):
                tmp[r] += syn0R[row1_R + r*topic_size + tp] * context_vector[tp]
    '''
    printf("tmp: \n")
    for r in range(rank):
        printf("%.2lf ",tmp[r])
    ...
    for r in range(rank):
        for v in range(vector_size):
            printf("%.2lf ",syn0L[row1_L + r * vector_size + v])
        printf("\n")
    #sgemv(&trans,&rank,&vector_size,&ONEF,&syn0L[row1_L],&rank,tmp,&ONE,&ONEF,neu1,&ONE)
    '''

    for v in range(vector_size):
        for r in range(rank):
            neu1[v] += syn0L[row1_L+ r*vector_size+ v] * tmp[r]
    '''
    printf("neu1\n")
    for v in range(vector_size):
        printf("%.2lf ",neu1[v])
    ...
    for tp in range(topic_size):
        if context_vector[tp] > 0:
            for r in range(rank):
                for v in range(vector_size):
                    syn0R[row1_R + r * topic_size + tp] += context_vector[tp] * work[v] * syn0L[row1_L + r*vector_size + v]

    for r in range(rank):
        for v in range(vector_size):
            syn0L[row1_L+r*vector_size + v] += tmp[r] * work[v]
...
cdef unsigned long long fast_sentence_sg_neg(
    ...
    REAL_t *syn0L,REAL_t * syn0R, REAL_t *syn1neg, const int rank,const int vector_size,const int topic_size,REAL_t* context_vector, const np.uint32_t word_index,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work,REAL_t *neu1,REAL_t*tmp,
...
def train_sentence_sg(model, sentence,topic_vector, alpha, _work,_neu1,_tmp):
    cdef int hs = model.hs
    cdef int negative = model.negative
    cdef int sample = (model.sample != 0)

    cdef REAL_t *syn0R = <REAL_t *>(np.PyArray_DATA(model.syn0R))
    cdef REAL_t *syn0L = <REAL_t *>(np.PyArray_DATA(model.syn0L))
    ...
    cdef REAL_t *tmp = <REAL_t *>(np.PyArray_DATA(_tmp))
        ...
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                if j == i:
                    continue
                if hs:
                    fast_sentence_sg_hs(points[j], codes[j], codelens[j], syn0L,syn0R, syn1,rank, vector_size,topic_size,context_vector, indexes[i], _alpha, work,neu1,tmp, word_locks)
                if negative:
                    next_random = fast_sentence_sg_neg(negative, cum_table, cum_table_len, syn0L,syn0R, syn1neg, rank,vector_size,topic_size,context_vector, indexes[j], indexes[i], _alpha, work,neu1,tmp, next_random, word_locks)
