...
try:
    #raise ImportError
    from word2mat_v4_inner import train_sentence_sg, train_sentence_cbow, FAST_VERSION
except ImportError:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    FAST_VERSION = -1

    def train_sentence_sg(model, sentence, context_vector,alpha, work=None,neu1=None,tmp=None):
        """
...
def train_sg_pair(model, word, context_index,topic_vector, alpha, learn_vectors=True, learn_hidden=True,
    ...
    l1L = model.syn0L[context_index].reshape(model.rank,model.vector_size)# input word (NN input/projection layer)
    l1R = model.syn0R[context_index].reshape(model.rank,model.topic_size)
    tmpR_topic = l1R.dot(topic_vector)
    l1 = l1L.T.dot(tmpR_topic)
    ...
    if learn_vectors:
        model.syn0L[context_index] +=  outer(neu1e,tmpR_topic).T.reshape(model.vector_size * model.rank)
        model.syn0R[context_index] +=  outer(topic_vector,neu1e).dot(l1L.T).T.reshape(model.topic_size*model.rank)
...
class Word2Mat(utils.SaveLoad):
    ...
    def __init__(
            ...
            sg=1, hs=1, negative=0, cbow_mean=0, rank=5, hashfxn=hash, iter=1, null_word=0,other_model=None):
        """
    ...
    def reset_weights_from_model(self):
        ...
        for key,word in self.vocab.items():
            ...
            if key in other_model:
                ...
                vec = zeros((self.rank,self.vector_size),dtype=REAL)
                for i in xrange(self.rank):
                    vec[i] = vec0
                self.syn0L[idx] = vec.reshape(self.rank* self.vector_size)
                self.syn0R[idx] = ones(self.rank*self.topic_size,dtype=REAL) / self.rank
            else :
                self.syn0L[idx] = self.seeded_vector_with_size(self.index2word[idx] + str(self.seed),self.vector_size*self.rank,self.vector_size)
                self.syn0R[idx] = ones(self.rank*self.topic_size,dtype=REAL) / self.rank

    def _do_train_job(self, job, alpha, inits):

        work, neu1,tmp,context_vector = inits
        tally = 0
        raw_tally = 0
        for sentence_id,sentence in job:
            if self.sg:
                tally += train_sentence_sg(self, sentence,context_vector, alpha, work,neu1,tmp)
            else:
                tally += train_sentence_cbow(self, sentence,context_vector, alpha, work, neu1,tmp)
    ...
    def train(self, sentences, total_words=None, word_count=0, chunksize=100, total_examples=None, queue_factor=2, report_delay=1):
        ...
        def worker_init():
            ...
            tmp = matutils.zeros_aligned(self.rank,dtype=REAL)
            context_vector = matutils.zeros_aligned(self.topic_size,dtype=REAL)
            return (work, neu1,tmp,context_vector)
    ...
    def __getitem__(self,word_context):
        ...
        return self.syn0L[self.vocab[word].index].reshape(self.rank,self.vector_size).T.dot(self.syn0R[self.vocab[word].index].reshape(self.rank,self.topic_size).dot(context_vector))
    ...
    def reset_weights(self):
        ...
        self.syn0L = empty((len(self.vocab), self.vector_size*self.rank), dtype=REAL)
        self.syn0R = empty((len(self.vocab), self.rank*self.topic_size), dtype=REAL)
        if self.other_model ==  None:
            # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
            for i in xrange(len(self.vocab)):
                # construct deterministic seed from word AND seed argument
                self.syn0L[i] = self.seeded_vector_with_size(self.index2word[i] + str(self.seed),self.vector_size*self.rank,self.vector_size)
                self.syn0R[i] = ones(self.rank*self.topic_size,dtype=REAL) / self.rank
        else :
    ...
    def seeded_vector_with_size(self,seed_string,size,norm):
        """Create one 'random' vector (but deterministic by seed_string)"""
        # Note: built-in hash() may vary by Python version or even (in Py3.x) per launch
        once = random.RandomState(uint32(self.hashfxn(seed_string)))
        return (once.rand(size) - 0.5) / norm
...
if __name__ == "__main__":
    ...
    sen = [[('1',0),('2',9),('43',2),('4',3),('5',2),('6',1)],[('2',0),('3',2),('4',1),('6',2),('7',1),('88',2),('8',1)],[('324',1),('34',2),('5',1),('6',3),('6',3),('3',2)]]
    topic_num = 10
    model = Word2Mat(sen,rank=5,size=10,topic=topic_num,min_count=0,iter=1000)
    print 'finish'
    context = zeros(topic_num,dtype=REAL)
    context[0] = 0.1
    context[1] = 0.2
    context[2] = 0.4
    context[3] = 0.3
